...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            #When the ghost is eaten we know that we have reached the ghost position so set belief to 1
            allPossible[self.getJailPosition()]=1.0
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            # if p is self.getJailPosition() and noisyDistance is None:
            #     allPossible[p] = 1
            if emissionModel[trueDistance] > 0:
                #P(A|B) ~= P(B|A)P(A) Conditional* prior
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                # Update belief of the new possible positions
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        #Distribute particles as positions uniformly
        self.particles = []
        for legal_pos in self.legalPositions:
            self.particles.extend([legal_pos]*(self.numParticles/len(self.legalPositions)))
        "*** YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            # When the ghost is eaten update to jailState
            for ind,particle in enumerate(self.particles):
                self.particles[ind] = self.getJailPosition()
            return
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            # P(A|B) ~= P(B|A)P(A) Conditional* prior
        ...
        if not allPossible.totalCount():
            ...
            resampleParticles = []
            for ind,pos in enumerate(self.particles):
    ...
    def getBeliefDistribution(self):
        ...
        beliefDist = Counter(self.particles)
        beliefDist = util.Counter(beliefDist)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        permut = list(itertools.product(self.legalPositions,self.legalPositions))
        random.shuffle(permut)
        self.particles = []
        for pos in permut:
            self.particles.extend([pos]*(self.numParticles/len(permut)))
        "*** YOUR CODE HERE ***"
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            #For each particle find prob if noisyDistance is None then set as jail state
            prob = 1.0
            for i in range(self.numGhosts):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                probDist = getPositionDistributionForGhost(setGhostPositions(gameState,newParticle),
                                                                i,
                                                                self.ghostAgents[i])
