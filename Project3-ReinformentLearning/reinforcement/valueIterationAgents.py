...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        #Set all values to 0.0 initially
        for state in mdp.getStates():
            self.values[state] = 0.0
        #Each Iteration find V_k and use it for next iteration by making a hard copy not symbolic copy
        for i in range(0,self.iterations):
            values_k = self.values.copy()#Significant change
            for state in self.mdp.getStates():
                ...
                for action in self.mdp.getPossibleActions(state):
                    ...
                    for next_state,prob in trans_prob:
                        v += prob*(self.mdp.getReward(state,action,next_state)+self.discount*values_k[next_state])
                    value_list.append(v)
